              ...
              {filterServices.map((service, index) => (
                <Tooltip key={index} delayDuration={0}>
                  <TooltipTrigger asChild>
                    <div className="relative filterSkillHover">
                      <Image
                        src={service.icon}
                        alt={service.title}
                        loading="lazy"
                      ...
                      {selectedServices.includes(service) && (
                        <div
                          className="absolute -top-1 -right-2 flex items-center justify-center transition-all cursor-pointer"
                          onClick={() => {
                            handleServicesSelect(service);
                          }}
                        >
                          <Check className="success size-4 p-0.5 text-white bg-primary-main rounded-full" />
                          <X className="error size-4 p-0.5 text-white bg-red-500 rounded-full" />
                        </div>
                      )}
                    </div>
                  </TooltipTrigger>
                ...
                >
                  ...
                  {project.year && (
                    <p className="text-gray-300 text-sm absolute bottom-14 left-4 flex items-center gap-2">
                      ...
                      {project.notPublic && (
                        <span className="bg-blue-500 text-white px-1.5 py-0.5 rounded-full text-xs flex items-center gap-1 ">
                      ...
                      <Tooltip key={index} delayDuration={0}>
                        <TooltipTrigger asChild>
                          <Image
                            ...
                            alt={chip?.title || ""}
                            loading="lazy"
                            className="w-[30px] hover:scale-110 transition-all cursor-pointer"
                          />
                        </TooltipTrigger>
                        <TooltipContent>
                          <p>{chip?.title}</p>
